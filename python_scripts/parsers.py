...
import gym
import numpy as np
from rlgym_sim.utils.action_parsers.action_parser import ActionParser
from rlgym_sim.utils.gamestates.game_state import GameState
from gym.spaces import Discrete

class SwappedNectoAction(ActionParser):
    def __init__(self):
        super().__init__()
...
class DashParser(ActionParser):
    ...
    def make_lookup_table():
        ...
        for steer in (-1, 0, 1):
            for boost in (0, 1):
                for handbrake in (0, 1):
                    actions.append([0, steer, 0, steer, 0, 0, boost, handbrake])
        # Aerial
        for pitch in (-1, 0, 1):
            for yaw in (-1, 0, 1):
                for roll in (-1, 0, 1):
                    for jump in (0, 1):
                        for boost in (0, 1):
                            ...
                            # Enable handbrake for potential wavedashes
                            handbrake = jump == 1 and (pitch != 0 or yaw != 0 or roll != 0)
                            actions.append([0, yaw, pitch, yaw, roll, jump, boost, handbrake])
        actions = np.array(actions)
