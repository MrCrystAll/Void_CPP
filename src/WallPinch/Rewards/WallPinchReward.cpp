...
#include <WallPinch/Rewards/WallPinchReward.h>

USE_WALLPINCH_REWARDS_NS;

void WallPinchReward::Reset(const RLGSC::GameState& initialState)
{
	this->pinchReward.Reset(initialState);
}

float WallPinchReward::GetReward(const RLGSC::PlayerData& player, const RLGSC::GameState& state, const RLGSC::Action& prevAction)
{
...
float WallPinchReward::Corner(float x, short xOrientation, short yOrientation)
{
	return xOrientation * x + 8064 * yOrientation;
}

Vec WallPinchReward::GetCornerIntersection(short xFwd, short yFwd, float xPos, float yPos)
{
	float xResult = 0;
	float yResult = 0;
...
void WallPinchReward::LogAll(Report& report, bool final, std::string name, float weight)
{
