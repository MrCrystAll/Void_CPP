...
#include <CeilingPinch/Rewards/CeilingPinchReward.h>

USE_CEILINGPINCH_REWARDS_NS;

void CeilingPinchReward::Reset(const RLGSC::GameState& initialState)
{
	this->pinchReward.Reset(initialState);
}

float CeilingPinchReward::GetReward(const RLGSC::PlayerData& player, const RLGSC::GameState& state, const RLGSC::Action& prevAction)
{
...
void CeilingPinchReward::LogAll(Report& report, bool final, std::string name, float weight)
{
